...
router.post('/', protect, async (req, res) => {
  try {
    ...
    const calculationResult = calculateScore({
      ...
      estimatedBins: event.estimatedBins,
      wetFill: req.body.wetFill,
      dryFill: req.body.dryFill,
      recycleFill: req.body.recycleFill
    })

    const log = await WasteLog.create({
      ...req.body,
      score: calculationResult.total,
      scoreBreakdown: calculationResult.breakdown
    })

    // Transition event to 'active' so BMC knows data is ready to be handled
    ...
    res.status(201).json({ 
      ...
      accuracyPercent: calculationResult.accuracyPercent,
      actualTotal: calculationResult.actualTotal,
